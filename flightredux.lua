...
store_dir = filesystem.store_dir() .. '/flightredux/'
resources_dir = filesystem.resources_dir() .. '/flightredux/'

if not filesystem.is_dir(resources_dir) then
    util.toast("ALERT: resources dir is missing! Please make sure you installed FlightRedux properly.")
end
...
gpws = true
menu.toggle(menu.my_root(), "GPWS", {"gpws"}, "", function(on)
    gpws = on
end, true)



trajectory_icon = directx.create_texture(resources_dir .. 'trajectory.png')
altimeter_outline = directx.create_texture(resources_dir .. 'altimeter.png') 
boresight = directx.create_texture(resources_dir .. 'boresight.png') 
line = directx.create_texture(resources_dir .. 'line.png')
circle = directx.create_texture(resources_dir .. 'circle.png')
contact = directx.create_texture(resources_dir .. 'contact.png')
        ...
        if player_vehicle ~= 0 and PED.IS_PED_IN_ANY_PLANE(players.user_ped()) or PED.IS_PED_IN_ANY_HELI(players.user_ped()) then
            ...
            for k,veh in pairs(all_vehicles) do
                ...
                if veh ~= player_vehicle and ENTITY.GET_ENTITY_HEALTH(veh) > 0 and ENTITY.HAS_ENTITY_CLEAR_LOS_TO_ENTITY(player_vehicle, veh, 17) then
                    ...
                    directx.draw_texture(contact, 0.005, 0.005, 0.5, 0.5, draw_pos.x, draw_pos.y, 0, hud_green)
                end
            end
        end
        util.yield()
...
while true do 
    ...
    if player_vehicle ~= 0 then
        ...
        if PED.IS_PED_IN_ANY_PLANE(players.user_ped()) or PED.IS_PED_IN_ANY_HELI(players.user_ped()) then
            ...
            directx.draw_text(0.195, 0.42, tostring(math.ceil(knts)) .. " KNTS", 4, 1.2, hud_green, false)
